...
@njit(cache=True)
def _simulate_one(mu, A, beta, state0, t_start, t_end, inject_t, inject_dim, inject_m, seed, max_events=2_000_000):
    """单次模拟。state0: 各源维在 t_start 时刻的衰减和(β e^{-β...} 之和)。
    返回 (times, dims) 达到 max_events 截断。"""
    np.random.seed(seed)
    K = len(mu)
    state = state0.copy()
    t = t_start
    ...
    n = 0
    inj_done = inject_t < t_start  # 若注入点在窗前,视为已注入
        ...
        if not inj_done and t >= inject_t:
            ...
            for _ in range(inject_m):
                if n < max_events:
                    ...
                    n += 1
            inj_done = True
        # λ_k(t) = μ_k + Σ_j A[j,k] state[j];两事件间 λ 单调衰减,λ(t) 是有效上界
        lam_k = mu + A.T @ state
