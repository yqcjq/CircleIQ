...
def fit_topic(job):
    ...
    out = {
        ...
        "beta1": res["beta1"], "beta2": res["beta2"],
        "beta_halflife_min": round(np.log(2) / res["beta1"] / 60, 1),
        "beta2_halflife_h": round(np.log(2) / res["beta2"] / 3600, 1),
        "mu": res["mu"].tolist(), "mu_edges": res["mu_edges"].tolist(),
        "A": res["A"].tolist(), "A1": res["A1"].tolist(), "A2": res["A2"].tolist(),
