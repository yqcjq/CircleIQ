...
《社交媒体-性别与婚育观(wb)2025年7月-2026年1月.zip》与《生育话题(wb)2025年7月-2026年1月.zip》(各 12GB)**是同一份数据的两个拷贝**——两者独立走完整管线后,行数(7,188,815)、用户数(2,099,778)完全一致,产出 parquet **逐字节相同**(MD5 `c744b903...`)。竞赛目录将其列为生育话题的两个不同文件。处理:目录中标记 `duplicate_of`,建模数据集(服务器)只保留《生育话题(wb)2025年7月-2026年1月》,所有统计按去重口径。
...
## 6. 全量处理统计(2026-07-27 实测,最终版)

- **总量**: 385 zip / 4648 CSV → 首轮 **56.6 分钟**(Mac 10 核,7 worker)+ 2 个脏数值 zip 修复重跑 38 分钟
- **有效数据(去重复 zip 后)**: **43,854,454 行** = 重大议题 29.06M + 热点事件 9.76M + 传播案例 5.03M;独立用户(各单元并集前)~2000 万量级
- **产出**: core 2.86GB(43 列建模数据,全量在服务器)/ text ~48GB(长文本,本地母本)
- **zip 内去重**: 丢弃重复抓取 5.1M+ 行;跨 zip 重复(生育话题)在边表/事件构建时再去
- **坏时间戳**: 0.02%;**情绪填充率**: 84.7%(缺列议题拉低,有列议题≈100%)
- **转发占比**: 传播案例 45.5% / 热点事件 43.5% / 重大议题因议题 0.4%-81% 不等
- **异常**: 无 0 行 zip;31 个 zip <1000 行(小事件);1 个重复 zip(§3.3)
