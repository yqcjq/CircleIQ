...
- **GBDT 的 h24 边际从筛选面板的"接近显著"缩水到明确不显著**。原因在分层里:GBDT 的
  38 维特征在 y=0 层(test 64% 样本)系统性输给 naive(0.254 vs 0.167)——它没有机制把"预测
...
从第一性原理看,大面板的误差由三块构成:零膨胀(test 锚点 64% y24=0,全面板 71%)、量级回归、爆发相变。
六条侧路各攻一块:
...
| GBDT+级联特征(近6/24h最大单根级联规模/份额) | 爆发由超级传播帖驱动 | 0.463(总);h6 边际转显著 | 边际有效:h6 vs naive 从不显著转显著(−0.015 [−0.028,−0.003]),爆发层不动(1.447 vs naive 1.419) |
