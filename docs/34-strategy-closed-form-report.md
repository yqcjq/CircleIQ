...
在 17 组对照上全部 |z|<2,**网格搜索退役**;②通道排序对 α ±20% 扰动的 Kendall τ 中位 0.81,
...
对真实拟合参数的对照(`mc_validation.json`,6 事件 × 2-3 通道共 17 组,覆盖 ρ 0.84→0.99):

![闭式vs MC](figures/34-closed_vs_mc.png)

17 组(6 事件,ρ 覆盖 0.84–0.99;一组重复已去重)全部落在 MC 的 95% 置信带内(|z| ≤ 1.63)。与旧网格搜索的关键差异:**闭式对超临界
...
| 闭式 vs Ogata thinning MC(真实参数 17 组) | 全部 \|z\|≤1.63 | ✓ |
| T→∞ 与分支公式代数一致 | 机器精度 | ✓ |
...
| MC 对照 | `mc_validation.json`(17 组去重后) |
