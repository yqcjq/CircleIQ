...
**日期**: 2026-07-27(数据补齐后最终版)· **数据**: 382 个数据单元(7 重大议题 + 263 热点事件 + 112 传播案例,4385 万帖,去重复 zip 后)
...
| K | 稳定边 | 稳定用户 | 说明 |
|---|---|---|---|
| 2 | 693,931 | 408,652 | 覆盖最广,仍含偶发同现 |
| **3** | **159,307** | **99,348** | **采用**:覆盖与稳定性的折中 |
| 4 | 74,911 | 50,100 | 更严,可作稳健性检验 |
| 6 | 28,987 | 22,507 | 深度稳定核心 |

K=3 含义:在**至少 3 个不同议题/事件**中都与他人产生转发互动。

**鲁棒性注**: 生育话题补入 719 万行(修复重跑)后,稳定用户数仅变化 +14(99,334→99,348),模块度不变——稳定圈结构对单议题数据增量高度鲁棒,佐证其反映的是真实跨议题组织而非采样噪声。

## 4. 稳定圈结构

![大小分布](figures/a_size_dist.png)

- **99,348 稳定用户 → 3,188 个稳定圈**,其中 ≥10 人的 370 个,top 圈 ~9,500 人
- **模块度 0.845**,3 种子 ARI 0.88-0.97(结构稳定,非算法伪影)
- 防泄漏版(仅用 2025-07-01 前互动):62,964 用户,模块度 0.884——供传播预测层严格防泄漏使用
...
**遗留/下一步**:
- ~~生育话题 2 个 zip 修复后增量重跑~~ **已完成**——数字变化可忽略(+14 用户),已按最终口径更新本报告
- 内容一致性评估(BGE embedding)在稳定圈上重算,对比单议题圈的 0.58 vs 随机 0.49(进行中)
- 稳定圈作为 Hawkes 维度进入传播预测层(阶段C,全量拟合运行中)
